...
def histogram_delta(features):
    ...
    for idx in pb_index:
        # Calcul du logarithme et gestion des erreurs
        try:
            ...
            if math.isinf(val):  # Si val est 'inf', on le remplace par 0
                val = 0
        except (ZeroDivisionError, ValueError):  # Gestion des divisions par zéro et valeurs négatives
            val = 0
        delta.append(val)
...
def get_mock_psth(data, features, t_pre, t_post, bin_width, good_clusters):
    """
    ...
    for cluster in good_clusters:
        ...
        for bin in range(len(features)):
            ...
            if bin-int(t_pre/bin_width)>0 and bin+int(t_post/bin_width)<len(features):
                if features[bin]['Mock_change']>0 and features[bin]['Condition']==1 :
